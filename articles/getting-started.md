...
Kaplan–Meier is nonparametric, so it is drawn with **steps**. Bernstein
polynomial survival is a continuous function of time, so model curves
use **lines**
...
![Kaplan-Meier by stage (steps) vs Bernstein PH at median age (smooth
dashed).](getting-started_files/figure-html/km-bp-overlay-1.png)
...
- For more covariate-specific ribbons → see the *Survival prediction and
  ggplot* vignette.
