...
library(spsurv)
library("data.table")
dat <- fread("../MC/po_llogis/data_po_llogis_899.txt")
head(dat)
mean(dat$status)
...
fitbe
new <- data.frame(numerical = c(mean(dat$numerical), mean(dat$numerical)),
                  categorical = c(0,1))

plot(sort(fitmle$y[,1]), survivor.spbp(fitmle, newdata = new), pch = 19, bty = 'n', ylim = c(0,1))
